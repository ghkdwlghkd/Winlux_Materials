...
async function callAjaxForHTML5(tempFile, sUploadURL) {
  return new Promise(async function (resolve, reject) {
    ...
    form.append("img", tempFile, encodeURIComponent(tempFile.name));
    const res = await fetch("/smarteditor_img_upload", {
      method: "post",
      headers: {},
      body: form,
    });
    const resJson = await res.json();
    resolve(setPhotoToEditor(resJson.responseText));
    // window.close();
  });
}
...
function onAjaxError(e) {
  ...
  alert(
    "[가이드]사진 업로더할 서버URL셋팅이 필요합니다.-onAjaxErrorr" +
